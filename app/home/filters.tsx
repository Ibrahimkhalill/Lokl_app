    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.sliderTrack}>
          ...
          <View style={[styles.sliderThumb, { left: `${distance * 10 - 2.5}%` as any }]} />
        </View>
        ...
        {['free', 'paid'].map((tier) => (
          ...
          >
            <View style={[styles.radio, planTier === tier && styles.radioActive]}>
              {planTier === tier && <Ionicons name="checkmark" size={14} color={Colors.black} />}
            </View>
            <Text style={styles.radioLabel}>{tier.charAt(0).toUpperCase() + tier.slice(1)}</Text>
          </TouchableOpacity>
        ))}
        ...
        <View style={styles.chipRow}>
          {PRICES.map((p) => (
            ...
            >
              <Text style={[styles.chipText, activePrice === p && styles.chipTextActive]}>{p}</Text>
            </TouchableOpacity>
          ))}
        </View>
        ...
        <View style={styles.chipGrid}>
          {AMENITIES.map((a) => (
            <TouchableOpacity
              key={a.label}
              style={[styles.amenityChip, activeAmenities.includes(a.label) && styles.chipActive]}
              onPress={() => toggleAmenity(a.label)}
            >
              <Ionicons
                name={a.icon as any}
                size={16}
                color={activeAmenities.includes(a.label) ? Colors.black : Colors.textSecondary}
              />
              <Text style={[
                styles.chipText,
...
const styles = StyleSheet.create({
  ...
  label: { color: Colors.text, fontSize: 15, fontWeight: '600', marginBottom: 14 },
  labelSub: { color: Colors.textSecondary, fontWeight: '400' },
  highlight: { color: Colors.primary },
  sliderTrack: {
    height: 6, backgroundColor: Colors.cardBorder,
    borderRadius: 3, marginBottom: 24, position: 'relative',
  },
